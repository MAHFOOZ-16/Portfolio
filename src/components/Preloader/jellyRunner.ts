...
import { Slider } from './jelly/slider';
import { CameraController } from './jelly/camera';
import { EventHandler } from './jelly/events';
import {
  DirectionalLight,
  HitInfo,
  LineInfo,
...
import { TAAResolver } from './jelly/taa';
import {
  AMBIENT_COLOR,
  AMBIENT_INTENSITY,
  AO_BIAS,
  ...
  SPECULAR_POWER,
  SURF_DIST,
} from './jelly/constants';
import { NumberProvider } from './jelly/numbers';

export async function initJellySlider(
  canvas: HTMLCanvasElement,
  onProgressLoaded: () => void,
) {
  const initialQuality = 1.25;
  const gpu = (navigator as Navigator & { gpu?: { getPreferredCanvasFormat(): unknown } }).gpu;
  if (!gpu) throw new Error('WebGPU is not supported');
  const presentationFormat = gpu.getPreferredCanvasFormat() as 'bgra8unorm' | 'rgba8unorm';
...
let qualityScale = Math.min(1.5, Math.max(0.3, initialQuality));
let width = Math.max(1, canvas.clientWidth * qualityScale);
let height = Math.max(1, canvas.clientHeight * qualityScale);
canvas.width = width;
...
function render(timestamp: number) {
  ...
  const mappedTargetX = ((simulatedMouseX - 0.4) / (0.9 - 0.4)) * (TARGET_MAX - TARGET_MIN) + TARGET_OFFSET;

  slider.setDragX(mappedTargetX);
  ...
  taaResolver.resolve(
    textures[currentFrame].sampled as Parameters<TAAResolver['resolve']>[0],
    frameCount,
    currentFrame,
  );
...
function handleResize() {
  ...
  width = Math.max(1, canvas.clientWidth * qualityScale);
  height = Math.max(1, canvas.clientHeight * qualityScale);
  canvas.width = width;
  canvas.height = height;
